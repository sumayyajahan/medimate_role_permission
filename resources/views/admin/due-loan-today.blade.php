...
$sql = "SELECT ua.member_id, lc.loan_id, lc.user_id, lc.last_collection_amount, lc.total_loan_repay_amount, lc.due_amount, lc.fine_imposed, lc.collection_date, lc.next_payment_date,lr.installment_amount, lr.repay_date FROM `loan_collection` AS lc LEFT JOIN loan_record AS lr ON lr.loan_id =  lc.loan_id LEFT JOIN user_account AS ua ON lc.user_id = ua.user_id  WHERE (lc.due_amount > 0 OR lr.repay_status = 0) AND lc.next_payment_date <= '" . date('Y-m-d') . "'";
...
<body>
                                    ...
                                    <div class="card-body">
                                        <form id="loan_collection">
                                            <div class="form-inline mb-4" style="justify-content:space-between">
                                                ...
                                                <label>Fine Added (BDT)</label>
                                                <input type="number" id="fine_imposed" class="form-control" readonly>
                                            ...
                                            <div>
                                                <button class="btn btn-danger pull-left" id="check-close">Close</button>
                                                ...
                                                <tbody>
                                                    ...
                                                    ?>
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    <button class="btn btn-primary" onclick="addCollection('<?= $loan_id ?>','<?= $due_amount ?>','<?= $installment_amount ?>','<?= $fine_imposed ?>');">Add Collection <i class="fa fa-plus-circle"></i></button></a>
    ...
    <script>
        ...
        function addCollection(loan_id, da, ia, fine_imposed) {
            var due_amount = parseFloat(da);
            var installment_amount = parseFloat(ia);
            $('#check').show();
            $('#due_amount').val(due_amount);
            $('#fine_imposed').val(fine_imposed);
            $('#loan_id').val(loan_id);
